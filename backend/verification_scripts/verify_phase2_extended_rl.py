...
class MockJob:
    def __init__(self, algo):
        self.id = f"mock_job_{algo.replace('-', '_')}"
        self.algorithm = algo
        self.config = {
            "epochs": 1,
            "learning_rate": 0.001,
...
def main():
    ...
    test_algos = ["Liquid-NN", "Decision-Transformer", "QR-DQN", "CQL", "GAIL"]
    
    for algo in test_algos:
        print(f"\n--- Testing Phase 2 Algo: {algo} ---")
        job = MockJob(algo)
        try:
            res = ExtendedRLEngine.train_extended_rl(
                job=job,
                df=df,
                features=features,
                db=db,
            ...
            if res is not None:
                model, path, metrics = res
                print(f"✅ SUCCESS: {algo}")
                print(f"  -> Saved Path: {path}")
                print(f"  -> Metrics: {metrics}")
            else:
                print(f"⚠️ {algo} returned None. This usually means it's gracefully stubbed due to missing advanced libraries.")
                
        except ImportError as ie:
            print(f"⚠️ MISSING LIBRARY for {algo}: {str(ie)}")
