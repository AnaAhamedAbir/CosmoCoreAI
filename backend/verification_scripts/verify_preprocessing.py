...
def verify_preprocessing(missing, outlier, scaling):
    print(f"\n=======================================================")
    print(f"🚀 Testing Preprocessing -> Missing: {missing}, Outlier: {outlier}, Scaling: {scaling}")
    print(f"=======================================================")
    ...
    if job_data['status'] == "COMPLETED":
        print(f"✅ Training PASSED with these preprocessing settings.")
        return True
    else:
...
if __name__ == "__main__":
    ...
    for c in configs:
        name = f"Miss:{c[0]}|Out:{c[1]}|Scale:{c[2]}"
        results[name] = verify_preprocessing(c[0], c[1], c[2])
