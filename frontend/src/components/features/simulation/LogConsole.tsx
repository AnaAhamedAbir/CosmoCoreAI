...
const LogConsole: React.FC<LogConsoleProps> = ({ logs, onClear, className }) => {
    ...
    return (
        <div className={`flex flex-col bg-white dark:bg-black border border-slate-200 dark:border-[#141414] rounded-lg shadow-2xl overflow-hidden font-mono text-sm ${className}`}>
            {/* Header / Toolbar */}
            <div className="flex items-center justify-between px-4 py-2 bg-slate-50/80 dark:bg-[#050505]/80 border-b border-slate-200 dark:border-[#141414] backdrop-blur">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex bg-slate-100 dark:bg-[#0A0A0A] rounded-md p-0.5">
                        {(['ALL', 'INFO', 'SUCCESS', 'WARNING', 'ERROR'] as const).map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={`px-3 py-1 text-xs rounded transition-colors ${filter === f ? 'bg-white text-slate-800 dark:bg-[#222] dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                            >
                                {f}
                            </button>
                        ))}
                    ...
                    <div className="h-4 w-[1px] bg-slate-200 dark:bg-[#0A0A0A] mx-1"></div>
            ...
            {/* Log Output */}
            <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar bg-slate-50/50 dark:bg-black/90 tracking-tight">
                {logs.length === 0 && (
                ...
                {filteredLogs.map((log, i) => (
                    <div key={i} className={`flex gap-3 hover:bg-slate-100 dark:hover:bg-[#050505]/30 px-1 rounded ${getLevelColor(log.level)}`}>
                        <span className="text-slate-500 dark:text-slate-600 shrink-0 select-none w-24 text-right">[{formatTime(log.timestamp)}]</span>
